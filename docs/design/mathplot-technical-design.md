...
| D7 | 隐式二元方程 | **数值探针系数提取**（`conic.ts` 纯函数分类器）＋ kind=`line` 参数化采样；二次 9 点探针 + 判别式分类交付抛物线/双曲线（ZOO-147）与退化两直线/单点/空集（ZOO-148），xy 旋转留 149 | 扩展标准形正则、marching squares 通用隐式 |
...
#### D7 续章：二次 9 点探针 + 判别式分类（ZOO-147，抛物线 / 双曲线）

**二次探针**（研究报告 §3）：F(x,y)=Ax²+Bxy+Cy²+Dx+Ey+F₀ 用 9 点对称差分精确恢复——
`A=(F(1,0)+F(−1,0))/2−F₀`、`C=(F(0,1)+F(0,−1))/2−F₀`、`D=(F(1,0)−F(−1,0))/2`、`E=(F(0,1)−F(0,−1))/2`、`B=(F(1,1)+F(−1,−1)−F(1,−1)−F(−1,1))/4`；校验点 (2,3)/(−2,−3)/(2,−3)/(−3,2)/(0.5,1.5) 相对容差比对（负象限点拆穿 |x|/|y| 伪装，三次与 sin 立即出局）。每方程共 14 次求值，µs 级。

**分类**（系数先按二次部分归一）：判别式 δ=B²−4AC（旋转不变量）——δ≈0（零带 1e-7）抛物线：A/C 恰一非零，配方可得顶点 (h,k) 与焦参数 p=−E/(4A)（axis='y'）或 −D/(4C)（axis='x'），覆盖平移与四开口方向，轴向线性系数近零判退化（平行直线族，ZOO-148）；δ>0 双曲线：中心 (−D/2A, −E/2C)，K=Ah²+Ck²−F 与 A 同号定实轴方向（a²=|K/实轴系数|、b²=|K/虚轴系数|），K≈0 判退化（相交直线）；δ<0 椭圆型一般式→引导标准形输入（ZOO-149 随旋转一并覆盖）；|B|>1e-9 →「含 xy 交叉项」引导文案（ZOO-149）。

**采样与视窗**：抛物线参数化 `y=k+t, x=h+t²/(4p)`（单支连续），双曲线两支 `x=h±a·cosh t, y=k+b·sinh t`（t 上限 arcosh 保证越出卡片，绘制层裁剪贯穿边缘）；视窗原点居中、纳入顶点/焦点（或中心/顶点/焦点），y 跨度 = 卡片高宽比 × x 跨度。

**等比视窗修复（本环重要修正，惠及全部几何 kind）**：原实现几何采样器返回「方形数学视窗」，而主画布默认卡片 480×360——`createPlotTransform` 按视窗映射外框，单位像素密度 unitPxY = 0.75·unitPxX，圆被画成椭圆、直线斜率失真（实测复现：`resolvePlotRender` 圆卡片 unit 比 4:3）。修复：`sampleGeometry` 增 `aspect`（=卡片高宽比，`computePlotRender` 注入）参数，各采样器经 `aspectWindow` 产纵横比一致且容纳全部锚点的视窗；`sampleEquation` 透传。单测以「任意卡片尺寸下 unitPxX === unitPxY」固化。MiniPreview（240×96）维持既有 fit 行为不变。

**SVG 导出裁剪**：几何 kind 采样刻意越出卡片（贯穿边缘），canvas 有 `ctx.clip` 而 SVG 无——`mathPlotToSvg` 曲线 path 增 `<clipPath>`（内嵌绘图区矩形），修复直线 SVG 导出溢出卡片（既有缺陷）。

**教学参数（探针路线独有收益）**：抛物线面板展示标准形 `(y-1)²=8(x+2)`、顶点/焦点/准线/开口方向；双曲线展示标准形、中心、a/b/c、两焦点、渐近线（平移形 `(y+2) = ±1.5(x-1)`）、准线 `x = 1±1.1094`、离心率。系数由 `validateEquation` 重解析取得（元素只存方程原文，不落盘派生字段，与 line 先例一致）。模板 +2（抛物线 `y²=4x`、双曲线 `x²/9-y²/4=1`）。

#### D7 续章二：退化二次曲线拆解（ZOO-148，两直线 / 单点 / 空集）

**判定与拆解**（在 ZOO-147 判别式分类的三个退化接入位落地，系数仍按二次部分归一）：

- **抛物线型缺轴向项**（δ≈0 且 E≈0 / D≈0）：残留单变量二次 `Av²+Pv+Q=0`（v=x 或 y，由 |A|≥|C| 分支定）——判别式（相对容差 1e-9）>0 两异根 → `kind='linePair'`（mode='parallel'，两条竖线 x=r₁/r₂ 或横线 y=r₁/r₂）；=0 重根 → mode='coincident'（单线）；<0 → 空集教学文案。
- **双曲线型 K≈0**（δ>0，中心配方余项相对容差）：`A(x−h)²=−C(y−k)²` 两侧同号 → `(x−h)=±√(−C/A)(y−k)`，两条**相交**直线过原中心 (h,k) → `kind='linePair'`（mode='intersecting'，一般式 x∓m·y=h∓m·k，m=√(−C/A)）。
- **椭圆型**（δ<0，本环新增拆解）：K=Ah²+Ck²−F——K≈0 平方和为零 ⟺ 退化**单点** (h,k) → `kind='point'`；K 与 A 异号（平方和=负数）→ **空集**教学文案；否则维持引导标准形（ZOO-149）。
- 非轴对齐直线对展开必含 xy 项 → 旋转形文案（ZOO-149），不在本环。

**行为定义**（研究报告 §3.2 建议）：两直线 → 出图（linePair 复用 line 全部设施，一个元素两条贯穿折线，共用同一原点居中视窗——基准半宽取两线锚点较大者，网格/轴上下文一致）；单点 → 出图（小圆标记：半径 = 视窗半宽 2%（下限 0.1），恒可见且随元素整体缩放，视窗纳入点与原点）；空集 → 错误占位元素 + 可解释文案（椭圆型「左侧恒正（或恒负）无法等于 0」、抛物线型「v 的二次式判别式小于 0 无实根」，均附示例）。

**配套修正**：圆标准形快路径 `x²+y²=0`（r=0）原报「圆的半径必须为正」，现交回隐式分类器 → 正确出退化单点；编辑器状态行 KIND_LABELS 补齐 parabola/hyperbola（ZOO-147 遗留缺口）与新增两 kind。

**教学参数**：linePair 面板展示形态标签（两条相交/平行/重合直线）+ 各线一般式 + 交点（联立即退化前中心）或间距 d=|c₁−c₂|/√(a²+b²)；point 展示坐标与唯一解 `x = …, y = …`。badge 用琥珀色与常规几何（蓝）区分。模板 +1（`x²-y²=0`）。

#### D7 续章三：含 xy 交叉项的坐标旋转 + 椭圆型一般式（ZOO-149，旋转圆锥曲线）

**旋转消交叉项**（研究报告 §3.3，Stage 8 末环）：|B|>1e-9（归一后）时坐标旋转 θ=½·atan2(B, A−C)——旋转坐标 u=x·cosθ+y·sinθ、v=−x·sinθ+y·cosθ 下二次部分对角化，特征值 λ₁,₂=(A+C)/2±R（R=√(((A−C)/2)²+(B/2)²)，u² 系数恒为代数较大的 λ₁）；判别式 δ=B²−4AC=−4λ₁λ₂ 旋转不变，沿用既容差带分类：

- **δ<0 椭圆型**：Cramer 解中心（2Ah+Bk+D=0、Bh+2Ck+E=0，det=−δ≠0），中心化余项 K'=F₀−(Ah²+Bhk+Ck²)——K'≈0 退化单点 (h,k)（relative 容差）；K' 与 λ 异号空集（沿用椭圆型空集文案）；否则 λ₁u²+λ₂v²=−K' 出椭圆，rx=√(−K'/λ₁) 沿 u（存 `rotation: θ`）、ry 沿 v。
- **δ>0 双曲线型**：同上中心与 K'——K'≈0 退化为两条过中心的相交直线（u=±m·v、m=√(−λ₂/λ₁)，映回 xy 一般式，`kind='linePair'` 复用全部设施）；否则与 −K' 同号的 λ 为实轴，a=√(|−K'/λ_t|) 沿实轴（`rotation` 规范到 (−90°,90°]）。
- **δ≈0 抛物线型**：λ₂≈0，旋转后 λ₁u²+D'u+E'v+F₀=0（D'=D·cosθ+E·sinθ、E'=−D·sinθ+E·cosθ）——E'≈0（相对容差）判平行线族（λ₁u²+D'u+F₀=0 求根，线 u=root 即 a=cosθ、b=sinθ，重合/空集同 ZOO-148 规则，空集文案改称「旋转坐标 u」）；否则配方 λ₁(u−u₀)²=−E'(v−v₀) 出抛物线，顶点映回 xy、p=−E'/(4λ₁)。**主轴角规范到 (−90°,90°] 时若平移了 π，带符号的 p 随轴翻转**（e₁ 反向、开口方向不变）。

**椭圆型一般式（B=0）同步放行**：ZOO-148 的「引导标准形输入」出口替换为直接出图——A(x−h)²+C(y−k)²=K 半轴 rx=√(K/A)、ry=√(K/C)，`rotation` 缺省（轴对齐，与 detectGeometry 标准形椭圆共用 `kind='ellipse'` 与教学面板）。

**数据模型**：`EllipseParams`/`HyperbolaParams`/`ParabolaParams` 增可选 `rotation`（弧度，主轴相对 x 轴转角；缺省 = 轴对齐，旧元素零迁移）。采样层椭圆参数点绕 (cx,cy) 旋转、AABB 半宽取轴向端点旋转投影极值（hypot(rx·|cosφ|, ry·|sinφ|)）；抛物线/双曲线以 e₁/e₂ 单位向量参数化（P=V+(t²/4p)e₁+t·e₂ / C±a·cosh·e₁+b·sinh·e₂），t 上限按卡片对角线放宽（贯穿即裁剪）；非旋转路径零改动。面板派生字段增 `ellipseParams`（PropertyPanel 经 validateEquation 重解析填充，元素不落盘派生值）。

**教学参数（旋转形）**：标准形在旋转坐标系 X'Y' 中表述（如 `X'²+Y'²/4=1`、`Y'²=1.41421X'`）+ 旋转角行（度数，如 `旋转角 θ = -45°（X' 为 rx 轴方向…）`）；焦点/顶点/中心给原坐标系坐标；渐近线与准线无法用斜截式表述的统一走一般式（首项系数归一，如 `x+y=1.41421`）。椭圆新增教学面板（标准形/中心/长短半轴/焦点/离心率，长轴=max(rx,ry)、圆时焦点重合于中心）。模板 +2（`xy=1` 旋转双曲线、`5x²-6xy+5y²=8` 旋转椭圆）。

**配套修正（既有缺陷）**：双曲线采样只取 t∈[0,tMax]，sinh≥0 导致**每支只画了上半臂**（x²/9−y²/4=1 实测下半臂缺失、ZOO-147 起存在，测试只验点在曲线上未验两臂）——改为 t 对称采样 ±tMax 修复，轴对齐与旋转形同时受益；新增「每支两臂分别越出卡片」断言固化。
...
**变换尺寸口径（ZOO-206 修复，重要）**：上表的 `width/height` 指**内嵌绘图区**尺寸（外框四周各让 `PLOT_INNER_PAD=6`，即 `width-12 / height-12`）——网格、轴、刻度、曲线折线回退、POI、SVG 导出与 **Path2D 构建变换**全部使用同一口径。ZOO-206 前 Path2D 误按完整外框构建，浏览器下所有方程相对坐标轴系统性偏移数 px（应过原点的方程不过原点）；Node 单测无 Path2D 走折线回退（口径一致）故未暴露。回归用例以「path2d 像素点经内嵌变换反解须落在真实曲线上」固化（`position.test.ts`）。
...
- **y 视窗自适应含 0 规则（ZOO-206）**：四分位稳健拟合后，若数据自身贴近 0（距 0 不超过其正向幅度的 10%），视窗扩展到含 0 并留 8% 边距——保证横轴与原点可见（`y=√x` 原点起笔可检验）。远离 0 的函数（`y=x²+100`）保持数据居中视窗不变。
