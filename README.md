...
- ƒ 方程工具（快捷键 `Alt+G`）：输入即校验、实时预览、13 个方程模板
...
常用快捷键：`Alt+V/P/R/O/L/A/T/X` 切换工具（画笔别名 `Alt+B`、圆别名 `Alt+C`），`Alt+G` 方程工具，`Ctrl+Z / Ctrl+Shift+Z` 撤销重做，`Delete` 删除选中，Space 拖拽平移画布，`Alt+/` 快捷键帮助。
