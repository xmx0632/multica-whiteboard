...
const KIND_BADGES: Record<MathPlotParamsValue['kind'], { label: string; cls: string }> = {
  ...
  linePair: { label: '两直线 · 退化', cls: 'bg-amber-50 text-amber-700' },
  point: { label: '单点 · 退化', cls: 'bg-amber-50 text-amber-700' },
        ...
        <>
          ...
          {linePair && (
            <div className="bg-amber-50/60 border border-amber-100 rounded-lg p-2 flex flex-col gap-1">
              <div className="text-[13px] font-semibold text-amber-700">退化：{linePair.label}</div>
          ...
          {degeneratePoint && (
            <div className="bg-amber-50/60 border border-amber-100 rounded-lg p-2 flex flex-col gap-1">
              <div className="text-[13px] font-semibold text-amber-700">退化：单点</div>
              <div className="font-serif text-[13px] text-gray-800">{degeneratePoint.point}</div>
              <div className="text-[11px] text-gray-500 leading-relaxed">唯一解：{degeneratePoint.solution}（图中以小圆点标记）</div>
            </div>
          )}
